...
| Bereich | Bedeutung |
|---|---|
| **Hausverbrauch** | Pflicht: archivierte Leistungsvariable. Einheit W/kW automatisch oder manuell. Bleibt das Feld leer und gibt es genau **einen MeterHub-Zähler mit der Funktion „Hausverbrauch“** (Leistung in W, archiviert, aktuell), wird dessen Leistung automatisch übernommen — das Formular zeigt Zähler, Variable und Vertrag („🔗 automatisch übernommen“). Mehrere solche Zähler werden nicht geraten (⚠️, Variable selbst wählen); ein Zähler, dessen Werte nicht wie eine positive Hauslast aussehen (Vorzeichen unklar), wird nicht automatisch genommen; eine eigene Variable hat immer Vorrang. Andere Quellen (abgeleiteter EMS-Wert, „Last gesamt“ des Wechselrichters) werden bewusst nicht automatisch genommen. |
...
| **Wallboxen automatisch abziehen** *(optional, Standard aus)* | Erkennt Wallboxen der NRG-Stack-Hubs (ChargerHub/OCPPHub) über deren Schnittstelle und zieht ihre **archivierte** Ladeleistung zusätzlich zur Abzugsliste ab. Jede Wallbox nur einmal (Abgleich über die Seriennummer, auch wenn sie über beide Hubs erreichbar ist). Nur einschalten, wenn die Hausverbrauch-Variable die Ladeleistung **enthält**; von Hand eingetragene Wallboxen dann aus der Liste entfernen, sonst wird doppelt abgezogen. Die Status-Zeile nennt erkannte und ignorierte Wallboxen. |
