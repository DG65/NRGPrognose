...
- **Prüfstände (lokal, `tools/pruefstand/`):** Vor Änderungen an Zeitraster, Tagesgrenzen,
  ...
  Abzugsliste/Wallbox-Erkennung `wallboxen.php`, an der Veraltet-Warnung `veraltet.php`, an Sondereffekt-Ausschluss (Lernmaterial/Kalibrierung, `affects`) `sondereffekte.php`, am automatischen Hausverbrauch (MeterHub `house`) `hausverbrauch.php`, an der Fehlerbehandlung um `AC_GetAggregatedValues` (defektes Archiv-Aggregat) `archivstoerung.php`, an Formular/Automatik-Erkennung (Statuszeilen, SUITE.md
